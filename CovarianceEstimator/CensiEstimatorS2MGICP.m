    ...
    methods
        ...
        function Sigma = compute(obj, pc1, pc2, T_est)
            ...
            for i = 1:n
                ...
                % Accumulate Gauss-Newton Hessian
                d2J_dX2 = d2J_dX2 + obj.buildH(p, Cp, q, Cq, eye(4));
            ...
            % METHOD 1) Better conditioning
            % H = d2J_dX2;
            % B = d2J_dZdX * bigZ * d2J_dZdX.';   % or the block-accumulated version above
            % 
            ...
            % 
            % % Sigma = H^{-1} B H^{-T}
            % R = chol(H);                 % Gauss-Newton H should be SPD near the solution
            % Y = R \ (R' \ B);
            % Sigma = (Y + Y.')/2;         % keep it numerically symmetric
